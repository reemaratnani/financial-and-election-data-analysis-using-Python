...
with open('election_data.csv', 'r') as csvfile:
    ...
    for row in csv_reader:
        i = row['Candidate']        
        if i in Vote_count:      # To keep the count of total votes of candidate and vote count set up to go up by 1 as candidate's name appears in row
            Vote_count[i] += 1  
        else:
...
print("------------------------")
print(f"Winner: {winner}")                     # Set to get the key(winner's name) with the maximum vote
print("------------------------")
